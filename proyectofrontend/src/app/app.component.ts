...
export class AppComponent {
  ...
  borrarVenta(id: number) {
    if (confirm("¿Realmente desea eliminar esta venta?")) {
      this.http.delete(`http://192.168.100.139:9090/api/venta/${id}`)
          .subscribe(
              () => {
  ...
  agregar() {
    ...
    this.http.post('http://192.168.100.139:9090/api/venta', ventaData, { headers })
      .subscribe(
        (data) => {
  ...
  editarVenta(venta: Venta) {
    this.http.put<Venta>(`http://192.168.100.139:9090/api/venta/${venta.id}`, venta)
      .subscribe(
        (data) => {
  ...
  ngOnInit() {
    this.http.get("http://192.168.100.139:9090/api/venta")
      .subscribe(
        resultado => {
